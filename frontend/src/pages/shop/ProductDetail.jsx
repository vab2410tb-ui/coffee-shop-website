...
const ProductDetailPage = () => {
    ...
    const isDisabled =
        isOutOfStock || isMaxedOut || quantity > availableToAdd || isAdding;
            ...
            <div className={productDetail.productImg}>
                {selectedVariant?.images?.length > 0 ? (
                    selectedVariant.images.map((img, index) => (
                        <img src={img || products.mainImage} alt={products.name} key={index} style={{ overflow: "hidden" }} />
                    ))
                ) : (
                    <div>
            ...
            <div className={productDetail.productInfo}>
                ...
                {validVariants && validVariants.length > 0 && (
                    <div style={{ marginTop: "15px" }}>
                        <p style={{ fontSize: "22px" }}> Color:{" "} <span style={{ fontSize: "22px" }}>{selectedVariant?.color}</span></p>
                ...
                <div style={{ display: "flex", gap: "15px", alignItems: "center", marginTop: "15px",  marginBottom: "15px"}}>
                    <div style={{ display: "flex", alignItems: "center", marginTop: "15px", marginBottom: "15px", border: "1px solid #D9D9D9", borderRadius: "40px", }} >
                        <button
                            onClick={handleDecrease}
                            disabled={quantity <= 1}
                            style={{ width: "40px", height: "40px", cursor: "pointer", background: "transparent", border: "none", fontSize: "20px", }} 
                        >
                            <FontAwesomeIcon icon={faAngleLeft} />
                        </button>
                        <span style={{ width: "50px", textAlign: "center", fontSize: "18px", paddingBottom: "3px", }}
                        >
                            {quantity}
                        </span>
                        <button
                ...
                {products?.description?.productFeatures?.map((item, index) => (
                    <li key={index} style={{ fontSize: "20px", color: "#646464", margin: "10px 0 0 20px",}}>
                        {item}
                    </li>
                ))}
